...
func (t *BashTool) Definition() schema.ToolDefinition {
	return schema.ToolDefinition{
		Name:        t.Name(),
		Description: "在当前工作区执行任意的 shell 命令。支持链式命令(如 &&)。返回标准输出(stdout)和标准错误(stderr)。",
		InputSchema: map[string]interface{}{
			"type": "object",
			"properties": map[string]interface{}{
				"command": map[string]interface{}{
					"type":        "string",
					"description": "要执行的命令，例如: ls -la 或 go test ./...",
				},
			},
...
func (t *BashTool) Execute(ctx context.Context, rawArgs json.RawMessage) (string, error) {
	...
	shellName, shellArgs := resolveShell(input.Command)
